...
import { SetParamsFromUrl } from "./components/SetParamsFromUrl";
...
export default async function ProductPage({
  ...
  params: { id: string };
  searchParams: { aid: string; t: string };
}) {
  const { id } = await params;
  const { aid, t } = await searchParams;
  ...
  return (
    <div>
      {(aid || t) && <SetParamsFromUrl aid={aid} t={t} />}
      <div>
        <div>
          <BackButton />
        </div>
      ...
      <div className="flex md:flex-row flex-col items-stretch gap-5 mt-5">
        ...
        <Card className="grow" contentClass="h-full flex flex-col">
          ...
          <div className="text-sm text-gray-400 mt-2">Артикул: {data.articul}</div>
          <ShareButton id={data.id} className="mt-2"/>
          <ProdDesc data={data} />
